function newRegister() {
    ...
    newP.appendChild(newText); // p노드가 상위요소가 되고 text라는 하위 요소를 연결
    ...
    delBtn.appendChild(delText); // 텍스트 노드가 button 하위요소로 연결하기
    ...
    // removeBtn 즉, 'X'의 전체를 반복한다.
    for(var i = 0; i < removeBtns.length; i++) {
        // removeBtn의 각각의 버튼에 이벤트를 등록하고 있다.
        removeBtns[i].addEventListener('click', function() {
            // #nameList를 의미하게 되는 것이다.
